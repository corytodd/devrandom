...
#pragma alloc_text (PAGE, DevRandomTimerCreate)
#endif

LONG
DevRandomInterlockedIncrementFloor(
...
FORCEINLINE
LONG
DevRandomInterlockedIncrementGTZero(
...
Return Value:
  ...
  --*/
{
    return DevRandomInterlockedIncrementFloor(Target, 0);
}
...
--*/
{
    ...
    queueConfig.EvtIoWrite  = DevRandomEvtIoWrite;
    ...
    //
    status = DevRandomTimerCreate(&queueContext->Timer, TIMER_PERIOD, queue);
    if (!NT_SUCCESS(status)) {
...
--*/
{
    ...
    // that on our own.
    //
    WDF_TIMER_CONFIG_INIT_PERIODIC(&timerConfig, DevRandomEvtTimerFunc, Period);
...
BOOLEAN
DevRandomIncrementRequestCancelOwnershipCount(
    PREQUEST_CONTEXT RequestContext
    )
/*++
  ...
  --*/
{
    ...
    //
    if (DevRandomInterlockedIncrementGTZero(
            &RequestContext->CancelCompletionOwnershipCount
            ) <= 1) {
        return FALSE;
    }
...
--*/
{
    ...
    KdPrint(("DevRandomEvtRequestCancel called on Request 0x%p\n",  Request));
    ...
    completeRequest = DevRandomDecrementRequestCancelOwnershipCount(requestContext);
    ...
    )
{
    ...
    //
    status = WdfRequestMarkCancelableEx(Request, DevRandomEvtRequestCancel);
    if (!NT_SUCCESS(status)) {
        queueContext->CurrentRequest = NULL;
    }
...
--*/
{
    ...
    Status = WdfMemoryCopyFromBuffer( memory, // destination
                             ...
                             Length );
    if( !NT_SUCCESS(Status) ) {
        KdPrint(("DevRandomEvtIoRead: WdfMemoryCopyFromBuffer failed 0x%x\n", Status));
    ...
    // request cancelable.
    //
    DevRandomSetCurrentRequest(Request, Queue);
...
--*/
{
    ...
    KdPrint(("DevRandomEvtIoWrite Called! Queue 0x%p, Request 0x%p Length %Iu\n",
                            Queue,Request,Length));

    if( Length > MAX_WRITE_LENGTH ) {
        KdPrint(("DevRandomEvtIoWrite Buffer Length to big %Iu, Max is %d\n",
    ...
    if( !NT_SUCCESS(Status) ) {
        KdPrint(("DevRandomEvtIoWrite Could not get request memory buffer 0x%x\n",
                 Status));
        WdfVerifierDbgBreakPoint();
        WdfRequestComplete(Request, Status);
        return;
    ...
    if( queueContext->Buffer == NULL ) {
        KdPrint(("DevRandomEvtIoWrite: Could not allocate %Iu byte buffer\n",Length));
    ...
    Status = WdfMemoryCopyToBuffer( memory,
                           ...
                           Length );
    if( !NT_SUCCESS(Status) ) {
        KdPrint(("DevRandomEvtIoWrite WdfMemoryCopyToBuffer failed 0x%x\n", Status));
        WdfVerifierDbgBreakPoint();
        ExFreePool(queueContext->Buffer);
        queueContext->Buffer = NULL;
        queueContext->Length = 0L;
...
--*/
{
    ...
    if (request != NULL) {
        ...
        if (DevRandomIncrementRequestCancelOwnershipCount(requestContext)) {
            cancel = TRUE;
        }
        else {
            //
    ...
    if (status != STATUS_CANCELLED) {
        ...
        completeRequest = DevRandomDecrementRequestCancelOwnershipCount(
            requestContext
            );
